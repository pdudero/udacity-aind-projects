...
class AirCargoProblem(Problem):
            ...
            for ap in self.airports:
                for p in self.planes:
                    for c in self.cargos:
                        precond_pos = [expr("At({}, {})".format(c, ap)),
                                       expr("At({}, {})".format(p, ap)),
                                       ]
                        precond_neg = []
                        effect_add = [expr("In({}, {})".format(c, p))]
                        effect_rem = [expr("At({}, {})".format(c, ap))]
                        load = Action(expr("Load({}, {}, {})".format(c, p, ap)),
            ...
            for ap in self.airports:
                for p in self.planes:
                    for c in self.cargos:
                        ...
                        unload = Action(expr("Unload({}, {}, {})".format(c, p, ap)),
                                      [precond_pos, precond_neg],
                                      [effect_add, effect_rem])
    ...
    def result(self, state: str, action: Action):
        ...
        poss=False
        for possact in self.actions(state):
            if str(action)==str(possact):
                poss=True
        assert(poss)
...
#            if clause in new_state.neg:
#                new_state.neg.remove(clause)
...
#               new_state.neg.append(clause)
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        for clause in self.goal:
            if clause not in kb.clauses:
                count += 1  # count up number of unmet goals
        return count
