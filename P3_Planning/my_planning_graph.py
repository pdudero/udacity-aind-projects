...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if newnode.prenodes.issubset(self.s_levels[level]):
                ...
                for node_a in self.a_levels[level]:
                    if node_a == newnode:
                        break
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level-1]:
            for effect_s in node_a.effnodes:
                self.s_levels[level].add(effect_s)
                # Now FIND the node in the set and connect
                for node_s in self.s_levels[level]:
                    if node_s == effect_s:
                        node_s.children.add(node_a)
                        node_a.parents.add(node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a1 in node_a1.action.effect_add:
            if effect_a1 in node_a2.action.effect_rem:
                return True
        for effect_a1 in node_a1.action.effect_rem:
            if effect_a1 in node_a2.action.effect_add:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a1 in node_a1.action.effect_add:
            if effect_a1 in node_a2.action.precond_neg:
                return True
        for effect_a1 in node_a1.action.effect_rem:
            if effect_a1 in node_a2.action.precond_pos:
                return True
        for effect_a2 in node_a2.action.effect_add:
            if effect_a2 in node_a1.action.precond_neg:
                return True
        for effect_a2 in node_a2.action.effect_rem:
            if effect_a2 in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Loop over sets of parents. All parents from one node must be mutex
        # with all parents from the other.
        for parent_a1 in node_a1.parents:
            for parent_a2 in node_a2.parents:
                if not parent_a1.is_mutex(parent_a2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for levelnum, s_level in enumerate(self.s_levels):
                goalreached = any([(goal==node.symbol and node.is_pos) for node in s_level])
                if goalreached:
                    level_sum += levelnum
                    break
...
#                    level_sum += levelnum
#                    goals.remove(goal)
#                    break  # Only one goal can be reached per level
        return level_sum
