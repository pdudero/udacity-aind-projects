...
class TestAirCargoMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.act2 = Action(
            expr('Unload(C1, P1, SFO)'),
            [[expr('In(C1, P1)'), expr('At(P1, SFO)')], []],
            [[expr('At(C1, SFO)')], [expr('In(C1, P1)')]]
        )
    ...
    def test_AC_result(self):
        print("\npossible actions:")
        possacts = self.p1.actions(self.p1.initial)
        poss=False
        for action in possacts:
            print(action)
            if str(self.act1)==str(action):
                poss=True
        print("\nact1:")
        print(self.act1)
        print(poss)
        ...
        fsstr = encode_state(fs, self.p1.state_map)
        fs = decode_state(self.p1.result(fsstr, self.act2), self.p1.state_map)
        self.assertTrue(expr('At(C1, SFO)') in fs.pos)
        self.assertTrue(expr('In(C1, P1)') in fs.neg)
